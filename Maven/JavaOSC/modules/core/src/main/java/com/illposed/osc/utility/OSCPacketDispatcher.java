...
public void dispatchPacket(OSCPacket packet,String remoteIp) {
		
		if (packet instanceof OSCBundle) {
			dispatchBundle((OSCBundle) packet,remoteIp);
		} else {
			OSCMessage message = (OSCMessage) packet;
			message.setRemoteIp(remoteIp);
	...
	public void dispatchPacket(OSCPacket packet, Date timestamp,String remoteIp) {
		if (packet instanceof OSCBundle) {
			dispatchBundle((OSCBundle) packet);
		} else {
			OSCMessage message = (OSCMessage) packet;
	...
	private void dispatchBundle(OSCBundle bundle,String remoteIp) {
		Date timestamp = bundle.getTimestamp();
		OSCPacket[] packets = bundle.getPackets();
		for (OSCPacket packet : packets) {
			dispatchPacket(packet, timestamp,remoteIp);
		}
	}
